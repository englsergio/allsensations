...
class WC_Rewards_Settings extends WC_Reward {
	...
	public function __construct() {
		...
		if (version_compare($woocommerce->version, '2.1.0') >= 0) {
			...
			add_action( 'woocommerce_update_options_rewards_' . $this->id, array( $this, 'process_admin_options' ) );
			add_action( 'woocommerce_update_options_rewards_' . $this->id, array( $this, 'save_table_values' ) );
	...
	function init() {
		...
		$settings = get_option( 'woocommerce_rewards_settings_settings' );
		$settings_updated = get_option( 'woocommerce_rewards_updated', 1 );
		
		if( $settings && version_compare( $settings_updated, '2.1.0') < 0 ){
			
			foreach( $settings['swr_roles'] as &$role ){
			...
			update_option( 'woocommerce_rewards_settings_settings', $settings );
			update_option( 'woocommerce_rewards_updated', $this->version );
		...
		if (version_compare($woocommerce->version, '2.1.0') >= 0) {
			...
			$this->swr_roles = $this->get_option('swr_roles', $this->roles);
			$this->rewards_page = $this->get_option('rewards_page', '');
			
			$this->swr_apply_rewards_to_shipping = $this->get_option('swr_apply_rewards_to_shipping', 'yes') == 'yes' ? true : false;
			$this->swr_apply_rewards_before_tax = $this->get_option('swr_apply_rewards_to_shipping', 'yes') == 'false' ? true : false;
			
			$this->payments = $this->get_option('payments', array());
			
			$this->table_values = get_option( 'woocommerce_rewards_points_table', array() );
			
			if( empty( $this->table_values ) && isset( $this->settings['table_values'] ) && !empty( $this->settings['table_values'] ) )
				$this->table_values = $this->settings['table_values'];
				
			if( isset( $_POST['points_required'] ) && !empty( $_POST['points_required'] ) ){
			...
			if( isset( $_POST['points_required'] ) && $gateways ){
				
				$this->payments = array();
				
				foreach( $gateways as $gateway ){
					
					$this->payments[$gateway->id] = $_POST[$gateway->id];
			...
			$this->swr_apply_rewards_before_tax = isset( $this->settings['swr_apply_rewards_before_tax'] ) ? ($this->settings['swr_apply_rewards_before_tax'] == 'yes' ? true : false) : false;
    ...
    public function auto_apply_rewards(){
	    return( $this->settings['swr_apply_rewards_auto'] == 'yes' ? true : false );
    }
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array( 
			...
			'swr_rewards_max_points' => array(
				'title' => __('Maximum reward', 'woocommerce'),
				'id' 		=> 'swr_rewards_max_points',
				'css' 		=> 'width:40px;',
				'type' 		=> 'text',
				'tip'	=>  __('Maximum reward that a customer can receive for a single order. Leave empty if no limit.', 'woocommerce')
			),
			'swr_rewards_min_points' => array(
				'title' => __('Minimum cash in points', 'woocommerce'),
		    ...
		    'swr_apply_rewards_auto' => array(  
				'title' 			=> __('Apply rewards auto', 'woocommerce'),
				'type' 				=> 'checkbox',
				'default' 			=> 'no',
				'tip'				=> __('Will automatically apply the customer rewards in the checkout.', 'woocommerce')
			),
			...
			'swr_apply_rewards_before_tax' => array(  
				'title' 			=> __('Apply rewards before tax', 'woocommerce'),
				'type' 				=> 'checkbox',
				'default' 			=> 'no',
				'tip'				=> __('Apply rewards money before tax', 'wc_rewards')
			),
		    ...
		    'text'           => array(
				'title'           => __( 'Texts', 'wc_rewards' ),
				'type'            => 'title',
				'description'     => ''
		    ),
			'swr_no_rewards_text' => array(
				'title' => __('Text no rewards', 'woocommerce'),
				'css' 		=> '',
				'type' 		=> 'text',
				'tip'		=>  __('Text that will be displayed when the order does not meet the minimum.', 'woocommerce')
			)
		);
    ...
    public function get_cart_applied_total(){
	    ...
	    if( $woocommerce->cart->cart_contents && ! empty( $woocommerce->cart->cart_contents ) ){
	    	...
	    	foreach( $woocommerce->cart->cart_contents as $content ){
		    	...
		    	$_product = $content[ 'data' ];
		    	$no_reward = get_post_meta( $_product->id, '_no_reward', true );
		    	
		    	if( $no_reward ){
			    	
			    	$line_price = $_product->get_price() * $content[ 'quantity' ];
			    	$minus += $line_price;
					
					if( $this->settings['swr_apply_type'] != 'subtotal' ){
						
						// Get item tax rates
						if ( empty( $tax_rates[ $_product->get_tax_class() ] ) )
							$tax_rates[ $_product->get_tax_class() ] = $woocommerce->cart->tax->get_rates( $_product->get_tax_class() );
	
						$item_tax_rates = $tax_rates[ $_product->get_tax_class() ];
	
						// Base tax for line before discount - we will store this in the order data
						$mtaxes = $woocommerce->cart->tax->calc_tax( $line_price, $item_tax_rates );
						$mtaxes = array_sum( $mtaxes );
						$minus += $mtaxes;
    	...
    	if($this->settings['swr_apply_type'] == 'totalwithoutshipping'){
    		if($this->rewards_to_rewards()){
	    		$amount = $woocommerce->cart->cart_contents_total + $woocommerce->cart->tax_total + ( version_compare($woocommerce->version, '2.3.0') >= 0 ? $woocommerce->cart->get_cart_discount_total() : $woocommerce->cart->discount_total );
    		}else{
	    		$amount = $woocommerce->cart->total - $woocommerce->cart->shipping_total + ( version_compare($woocommerce->version, '2.3.0') >= 0 ? $woocommerce->cart->get_cart_discount_total() : $woocommerce->cart->discount_total );
    		}
    ...
    public function get_product_extra_rewards( $product, $nb = 1, $include_amount = true, $wooprice = false ){
		...
		$extra_reward = 0;
		if( ! $no_reward ){
			if( get_class( $product ) == 'WC_Product' || get_class( $product ) == 'WC_Product_Simple' ){
	    		...
	    		if( isset( $product->variation_id ) ){
	    			...
	    			if( method_exists( $product, 'variable_product_sync' ) ){
						...
						$variation_id = $product->min_price_variation_id;
	    			}else{
		    			$variation_id = $product->variation_id;
	    			}
		    		
	    		}
		    	$extra_reward = get_post_meta( $variation_id, '_reward', true );
	    	}
		}
    ...
    public function get_max_points(){
	    return $this->settings['swr_rewards_max_points'];
    }
    ...
    public function get_no_rewards_text(){
	    return isset( $this->settings['swr_no_rewards_text'] ) ? $this->settings['swr_no_rewards_text'] : '';
    }
    ...
    public function get_order_amount( $order ){
	    ...
	    $total_shipping = method_exists( $order, 'get_total_shipping' ) ? $order->get_total_shipping() : $order->get_shipping();
	    ...
	    switch($this->settings['swr_apply_type']){
		    ...
		    case 'subtotal':
		    	if($this->rewards_to_rewards()){
		    		$amount = $order->get_total()+$order->get_total_discount()-$total_shipping-$order->get_total_tax();
		    	}else{
		    		$amount = $order->get_total()-$order->get_total_tax()-$total_shipping;
		    	}
		    	break;
		    case 'totalwithoutshipping':
		    	if($this->rewards_to_rewards()){
		    		$amount = $order->get_total()+$order->get_total_discount()-$total_shipping-$order->get_shipping_tax();
		    	}else{
		    		$amount = $order->get_total()-$total_shipping-$order->get_shipping_tax();
		    	}
		    	break;
	    }
    ...
    public function calculate_payments_extras( $payment_method, $amount ){
	    ...
	    if( ! isset( $_POST[ 'payment_method' ] ) || ! isset( $this->payments[ $_POST[ 'payment_method' ] ] ) )
	    	return false;
	...
	function generate_table_values_html ( $key, $data ) {
		...
		<tr valign="top" id="table_points_value">
			<th scope="row" class="titledesc"><?php _e( 'Points', 'wc_rewards' ); ?></th>
			<td class="forminp">
     ...
     */
    public function save_table_values() {
    	$values = array();
    	...
    	update_option( 'woocommerce_rewards_points_table', $values );
    }
